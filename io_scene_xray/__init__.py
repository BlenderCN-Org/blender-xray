...
def get_sumodules_old():
    path, f_name = os.path.split(__file__)

    # modules_names = [os.path.join(root, name)
    modules_names = [name.split(".py")[0]
                     for root, dirs, files in os.walk(path)
                     for name in files
                     if name.endswith(".py")]

    modules_names = [name.split(".py")[0] for name in os.listdir(path) if name.endswith(".py") and "__init" not in name]

    sub_modules = []

    for name in modules_names:
        try:
            submod = importlib.import_module(__package__ + "." + name)
